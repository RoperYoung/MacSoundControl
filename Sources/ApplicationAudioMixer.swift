//
//  ApplicationAudioMixer.swift
//  MacSoundControl
//
//  Portions adapted from MacMix v1.5 (commit b62914f).
//  Copyright (c) 2026 ljmng7(Jazmín), licensed under MIT.
...
nonisolated final class AppAudioMixer: @unchecked Sendable {
    ...
    private let lifecycleQueue = DispatchQueue(
        label: "MacSoundControl.AppAudioMixer.Lifecycle",
        qos: .userInitiated
    )
    private let retirementQueue = DispatchQueue(
        label: "MacSoundControl.AppAudioMixer.Retirement",
        qos: .userInitiated,
    ...
    func probeSystemAudioPermission(
        completion: @escaping @Sendable (Bool) -> Void
    ) {
        lifecycleQueue.async { [self] in
            completion(probeSystemAudioPermission())
        }
    }
    ...
    func submitQuiesceForOutputSwitch(
        ...
        lifecycleQueue.async { [self] in
            ...
            if !guardedAudioObjectIDs.isEmpty {
    ...
    private func probeSystemAudioPermission() -> Bool {
        ...
        tapDescription.name = "MacSoundControl Permission Request"
    ...
    private func apply(
        ...
        guard let engine = ProcessTapGainEngine(
...
nonisolated private final class ProcessTapMuteGuard {
    ...
    init?(audioObjectIDs: [AudioObjectID]) {
        ...
        tapDescription.name = "MacSoundControl Route Switch Guard"
        ...
        let aggregateDescription: [String: Any] = [
            kAudioAggregateDeviceNameKey: "MacSoundControl Route Switch Guard",
            kAudioAggregateDeviceUIDKey: "MacSoundControl.RouteGuard.\(UUID().uuidString)",
...
nonisolated private final class ProcessTapGainEngine: AppGainEngine {
    ...
    init?(
        ...
        let aggregateDescription: [String: Any] = [
            kAudioAggregateDeviceNameKey: "MacSoundControl Mixer",
            kAudioAggregateDeviceUIDKey: "MacSoundControl.Mixer.\(UUID().uuidString)",
            kAudioAggregateDeviceIsPrivateKey: true,
    ...
    private static func createProcessTap(
        ...
        let stereoMixdownTap = Self.configure(
            ...
            name: "MacSoundControl Stereo Mixdown"
        )
        ...
        if let outputStreamIndex = preferredMultichannelOutputStreamIndex(outputDeviceUID: outputDeviceUID) {
            let outputStreamTap = Self.configure(
                ...
                name: "MacSoundControl Output Stream \(outputStreamIndex)"
            )
            candidates = [
                ProcessTapConfiguration(
